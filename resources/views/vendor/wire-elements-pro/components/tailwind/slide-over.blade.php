...
<form class="flex h-full flex-col" wire:submit.prevent="{{ $onSubmit }}" x-on:keyup.shift.enter="{{ $onSubmit }}">
    ...
    <div @class(['relative mt-6 flex-1', 'px-4 sm:px-6' => $contentPadding])>
        <div @class(['absolute inset-0 h-full flex flex-col justify-between', 'px-4 sm:px-6' => $contentPadding])>
            {{ $slot }}
